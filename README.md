...
1. **开始游戏** —— 创建新的游戏进程。
2. **继续游戏** —— 载入 `saves` 目录中的 `autosave` 存档并进入游戏，如果该文件不存在则无法继续。
3. **健康检查** —— 跳转到 `/api/health` 查看服务器状态。
4. **性能指标** —— 打开 `/metrics` 显示 Prometheus 监控数据。

右侧的 **设置** 按钮可开启设置模态框；在游戏界面右上角也有齿轮图标可再次打开。
