...
def process_duplicates(duplicates: Dict[str, List[Path]], apply: bool = False) -> Dict[str, Any]:
    """处理重复文件，返回操作统计"""
    stats = {"groups": 0, "files_to_move": 0, "operations": []}
    ...
    for file_hash, files in duplicates.items():
        ...
        for filepath, _ in files_with_time[1:]:
            ...
            operation = {"action": "move", "source": str(filepath), "destination": str(dest_path)}
...
def main():
    ...
    if not apply:
        ...
        with open(log_file, "w") as f:
            json.dump({"timestamp": datetime.now().isoformat(), "stats": stats}, f, indent=2)
        print(f"\n操作日志已保存到: {log_file}")
