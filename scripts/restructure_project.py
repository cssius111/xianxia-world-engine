...
class ProjectRestructurer:
    ...
    def analyze_current_structure(self):
        ...
        # 统计各类JSON文件
        json_stats = {"total": 0, "by_directory": {}, "by_category": {}}
    ...
    def execute_restructure(self, plan: Dict, dry_run: bool = True):
        ...
        if dry_run:
            ...
            backup_dir = (
                self.project_root
                / f"backup_restructure_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        ...
        for move_info in plan["file_moves"][:10]:  # 显示前10个
            ...
            if dry_run:
                print(
                    f"  [DRY RUN] {source.relative_to(self.project_root)} → {target.relative_to(self.project_root)}"
                )
            else:
                try:
                    # 备份原文件
    ...
    def _generate_restructure_report(self, plan: Dict, dry_run: bool):
        """生成重构报告"""
        report_name = (
            f"restructure_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        if dry_run:
