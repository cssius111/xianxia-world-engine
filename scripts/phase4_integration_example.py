...
from xwe.metrics import inc_counter, metrics_registry, time_histogram
...
def on_player_login(player_id: str):
    ...
    inc_counter(
        "game_events_total",
        1,
        {"event_type": "player_login", "category": "auth"},  # 使用类别代替玩家ID
    )

    # 记录结构化日志
    logger.info("Player logged in", player_id=player_id, category="auth")
...
def on_combat_start(player_id: str, enemy_id: str):
    ...
    with time_histogram("combat_processing_seconds", {"type": "start"}):
        # 记录事件（服务级别标签）
        inc_counter("game_events_total", 1, {"event_type": "combat_start", "category": "combat"})

        logger.info("Combat started", player_id=player_id, enemy_id=enemy_id, category="combat")
...
def handle_error(error):
    """全局错误处理"""
    logger.error("Unhandled exception", error=error, path=request.path)

    # 记录错误指标
    inc_counter("errors_total", 1, {"error_type": type(error).__name__, "severity": "error"})
...
def example_with_services():
    ...
    # 监控Service调用
    with time_histogram(
        "service_call_duration_seconds", {"service": "game", "method": "process_command"}
    ):
