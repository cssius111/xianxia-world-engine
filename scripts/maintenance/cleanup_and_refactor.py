...
class ProjectRefactorer:
    def __init__(self, project_root: str):
        ...
        self.backup_dir = (
            self.project_root
            / f"backup_refactor_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        self.actions_log = []
    ...
    def analyze_project(self):
        ...
        for old_path, new_path, desc in duplicate_pairs:
            ...
            if old_full.exists() and new_full.exists():
                issues["duplicate_modules"].append(
                    {"old": str(old_full), "new": str(new_full), "description": desc}
                )
        ...
        # 3. 检查错误放置的文件
        root_files = [
            f for f in self.project_root.iterdir() if f.is_file() and f.suffix == ".py"
        ]
        ...
        for pattern in obsolete_patterns:
            for file in self.project_root.rglob(pattern):
                if not any(
                    skip in str(file) for skip in [".git", "__pycache__", "backup"]
                ):
    ...
    def _print_analysis_results(self, issues: Dict):
        ...
        total_issues = sum(
            len(v) if isinstance(v, list) else len(v) for v in issues.values()
        )
    ...
    def create_refactor_plan(self, issues: Dict) -> Dict:
        ...
        plan["create_files"].append(
            {"path": "start_web.py", "content": self._generate_new_launcher()}
        )
