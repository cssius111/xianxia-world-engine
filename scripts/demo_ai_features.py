...
from xwe.core.nlp.advanced import AdvancedPromptEngine, GameContext, ResponseType
    ...
    for player_input in test_conversations:
        ...
        try:
            ...
            result = await dialogue_manager.generate_npc_dialogue(
                context["npc"]["id"], player_input, context
            )
...
def demo_dialogue_mock():
    ...
    responses = [
        ...
        {"text": "好嘞！10颗回灵丹，一共450灵石。道友慢走，下次再来！", "emotion": "satisfied"},
    ]
...
async def demo_narrative_generation():
    ...
    context = {"location": {"name": "熔岩洞穴"}, "player": {"name": "道友", "realm": "筑基期"}}
    ...
    try:
        # 有发现的探索
        discovery = {"type": "灵药", "description": "千年雪莲", "value": "珍稀"}

        narrative = await narrator.generate_exploration_narrative(
            "仔细搜索洞穴深处", discovery, context
        )
        print(narrative)
...
async def demo_world_events():
    ...
    for severity in ["minor", "major"]:
        ...
        try:
            event = await event_generator.generate_world_event(f"定期{severity}事件", severity)
