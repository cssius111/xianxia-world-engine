...
def example_event_driven():
        ...
        def on_enemy_defeated(self, event):
            ...
            if self.achievements[key] == 10:
                self.log_service.log_achievement(
                    f"获得成就：{enemy_type}杀手（击败10个{enemy_type}）", player_id=player_id
                )

                # 发布成就事件
                event_dispatcher.dispatch_game_event(
                    "achievement_unlocked",
                    {"player_id": player_id, "achievement": f"{enemy_type}_slayer", "count": 10},
                )
    ...
    for i in range(12):
        event_dispatcher.dispatch_combat_event(
            "enemy_defeated", {"player_id": "player_001", "enemy_type": "妖兽", "enemy_level": 5}
        )
