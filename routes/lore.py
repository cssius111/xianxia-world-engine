...
def get_lore():
    ...
    try:
        ...
        if not lore_path.exists():
            return jsonify({"success": False, "error": "世界观文件未找到"}), 404
...
def get_lore_file(filename):
    ...
    try:
        ...
        if not lore_path.exists():
            return jsonify({"success": False, "error": f"剧情文件 {filename} 未找到"}), 404
...
def list_lore_files():
    ...
    try:
        ...
        for file_path in lore_dir.glob("*.md"):
            files.append(
                {"filename": file_path.name, "title": file_path.stem.replace("_", " ").title()}
            )
