...
def get_system_stats():
    ...
    return {
        ...
        "sessions": {"active": 1, "total": 1},  # 当前简化实现
    }
...
def health_check():
    ...
    try:
        ...
        if os.path.exists(saves_dir) and os.access(saves_dir, os.W_OK):
            checks["storage"] = "ok"
            details["storage"] = {"path": saves_dir, "writable": True}
        else:
            checks["storage"] = "error"
            details["storage"] = {"path": saves_dir, "error": "Directory not writable"}
...
def get_game_time():
    ...
    # 游戏时间（如果有的话）
    game_time = 0
    game_date = {"year": 1, "month": 1, "day": 1, "hour": 0, "season": "春"}
    ...
    if game and hasattr(game, "game_time"):
        ...
        game_date = {"year": year, "month": month, "day": day, "hour": hours, "season": season}

    return {"real_time": int(time.time()), "game_time": game_time, "game_date": game_date}
...
def get_metrics():
    ...
    return Response(metrics_registry.export_metrics(), mimetype="text/plain; version=0.0.4")
