...
from xwe.services import IGameService, ILogService, get_service_container
...
def get_game_status():
    ...
    if not game_state.initialized:
        return {"initialized": False, "message": "游戏尚未初始化"}
    ...
    if player:
        status_data["player"] = {
            ...
            "experience": {"current": player.experience, "required": player.experience_to_next},
            "attributes": {
...
def execute_command():
    ...
    try:
        ...
        log_service.log_error(
            f"Command execution error: {e}", category="command_error", metadata={"command": command}
        )
        ...
        raise InvalidCommandError(
            command=command, suggestions=suggestions or ["帮助", "状态", "地图"]
        )
...
def get_game_log():
    ...
    if level:
        try:
            ...
            raise InvalidRequestError(f"无效的日志级别: {level}", details={"level": level})
    if category:
    ...
    return {"logs": formatted_logs, "total": stats["total_logs"], "statistics": stats}
...
def get_recent_events():
    ...
    # 获取事件历史
    events = event_dispatcher.get_event_history(event_type=event_type, limit=limit)
...
@validate_request(
    {
        "type": "object",
        "properties": {"player_name": {"type": "string", "minLength": 2, "maxLength": 20}},
    }
)
def initialize_game():
    """
    ...
    if not success:
        return {"success": False, "message": "游戏初始化失败"}

    response = {"success": True, "message": "游戏初始化成功", "player_created": bool(player_name)}
...
def get_help():
    ...
    if command:
        ...
        return {"help": help_result.output, "command": command}
    else:
        # 获取所有命令
