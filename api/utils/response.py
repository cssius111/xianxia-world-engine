...
def api_response(f: Callable) -> Callable:
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            # 构建成功响应
            response_data = {"success": True, "data": result, "meta": _build_meta(start_time)}
...
def success_response(data: Any = None, message: str = None) -> tuple:
    ...
    """
    response = {"success": True, "data": data, "meta": _build_meta(time.time())}
...
def error_response(error: APIError) -> tuple:
    ...
    Returns:
        (response_dict, status_code)
    """
    response = {"success": False, "error": error.to_dict(), "meta": _build_meta(time.time())}
