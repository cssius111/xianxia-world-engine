...
def show_statistics(roll_count, best_character, best_score):
    ...
    if best_character:
        print(
            f"最佳评级: {best_character['overall_rating']['rank']} - {best_character['overall_rating']['description']}"
        )
    print("=" * 50)
...
def demo_mode():
    ...
    for i in range(5):
        ...
        print(
            f"体质: {character.physique.get('name', '凡体') if hasattr(character, 'physique') else '凡体'}"
        )
...
def batch_statistics(count=1000):
    ...
    for i in range(count):
        ...
        # 统计特殊体质
        if hasattr(character, "physique") and character.physique.get("rarity") in [
            "rare",
            "legendary",
        ]:
