...
from xwe.core.optimizations import AsyncEventSystem, ExpressionJITCompiler, SmartCache
...
def test_expression_jit():
    ...
    expressions = {
        "simple_damage": {
            "operation": "*",
            "operands": [{"attribute": "player.attack"}, {"constant": 2.5}],
        },
        "complex_damage": {
            "operation": "+",
            "operands": [
                {
                    ...
                    "operands": [
                        ...
                        {
                            "operation": "max",
                            "operands": [{"attribute": "skill.level"}, {"constant": 1}],
                        },
                    ],
                },
                {
                    ...
                    "operands": [
                        ...
                        {
                            "operation": "*",
                            "operands": [{"attribute": "target.armor"}, {"constant": 0.5}],
                        },
                    ],
                },
            ],
    ...
    # 测试上下文
    test_contexts = [
        {"player": {"attack": 100}, "skill": {"level": 5}, "target": {"defense": 50, "armor": 20}},
        {
            "player": {"attack": 200},
            "skill": {"level": 10},
    ...
    for expr_id, expression in expressions.items():
        ...
        result = benchmark.benchmark_expression(
            expr_id, expression, test_contexts, iterations=10000
        )
...
async def test_async_event_system():
    ...
    event_system.register_handler(
        "cpu_event", cpu_intensive_handler, is_async=False, is_cpu_intensive=True
    )
    ...
    event_system.register_batch_handler("batch_event", batch_handler, batch_size=50, max_wait=0.1)
...
async def test_combined_optimization():
        ...
        "operands": [
            {"attribute": "attacker.power"},
            {
                ...
                "operands": [
                    ...
                    {
                        "operation": "/",
                        "operands": [{"attribute": "target.defense"}, {"constant": 100}],
                    },
                ],
            },
        ],
    ...
    def calculate_battle_result(attacker_id: str, target_id: str):
        ...
        context = {"attacker": attacker, "target": target}

        # 使用JIT编译的函数计算伤害
        damage = damage_calc(context)

        return {"attacker": attacker_id, "target": target_id, "damage": damage}
    ...
    async def battle_handler(event):
        ...
        result = cache.get_or_compute(
            key, calculate_battle_result, event.data["attacker"], event.data["target"]
        )
    ...
    for _ in range(1000):
        ...
        await event_system.emit("battle", {"attacker": attacker["id"], "target": target["id"]})
