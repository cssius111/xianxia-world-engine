...
def test_formula_engine():
    ...
    try:
        ...
        # 测试带变量的表达式
        result = evaluate_expression("health * 0.1 + base_regen", {"health": 1000, "base_regen": 5})
        print(f"✓ 变量表达式: health * 0.1 + base_regen = {result}")
        assert result == 105

        # 测试预定义公式
        result = calculate("hit_chance", accuracy=80, evasion=30, level_difference=5)
    ...
    try:
        ...
        class TestPlayer:
            def __init__(self):
                ...
                self.spiritual_root = {"type": "single", "quality": "medium", "element": "fire"}
                self.cultivation = {"total_hours": 100, "breakthrough_count": 0, "failure_count": 0}
        ...
        print(
            f"✓ 突破{'成功' if breakthrough_result['success'] else '失败'}: {breakthrough_result['message']}"
        )
    ...
    try:
        ...
        class TestCombatant:
            def __init__(self, name, team=0):
                ...
                self.equipment = {"weapon": {"damage": 30, "critical_rate": 0.05}}
                self.element = "fire"
                self.realm = "qi_gathering"
                self.status_effects = []
                self.passive_skills = []
        ...
        # 玩家攻击
        action = {"action": combat_system.ActionType.ATTACK, "target": enemy.id}
