...
from xwe.world import EventSystem, LocationManager, WorldMap
...
def test_character_system():
    ...
    print(
        f"受到30点伤害后: {character.attributes.current_health}/{character.attributes.max_health}"
    )
...
def test_command_parser():
    ...
    test_commands = ["攻击 妖兽", "使用 剑气斩 攻击 敌人", "防御", "状态", "修炼"]
...
def test_combat_system():
    ...
    action = CombatAction(
        action_type=CombatActionType.ATTACK, actor_id=player.id, target_ids=[enemy.id]
    )
