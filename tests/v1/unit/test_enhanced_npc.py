...
import logging
from xwe.npc import (
    DialogueSystem, NPCManager, NPCProfile, NPCBehavior,
    EmotionType, PersonalityTrait
)
...
def demo_enhanced_dialogue():
    ...
    alchemist_dialogue = {
        "nodes": [
            ...
            },
            {
                ...
                "effects": {
                    "add_flag": "learned_special_technique",
                    "give_item": "alchemy_manual"
                }
            },
            {
                "id": "warn",
    ...
    # 赞美NPC
    print("\n玩家输入: 您的炼丹技术真是太厉害了！")
    npc_manager.emotion_system.trigger_emotion("npc_zhang_alchemist", "praise", {
    ...
    npc_manager.memory_system.create_memory(
        "npc_zhang_alchemist", player_id, "helped",
        game_time=100,
        player_name="道友"
    )
...
def demo_personality_effects():
    ...
    for name, template, desc in personalities:
        ...
        profile = NPCProfile(
            ...
            name=name,
            description=desc,
            extra_data={'personality_template': template}
        )
        ...
        if personality:
            ...
            print(f"对话风格: 正式度={style['formality']:.2f}, "
