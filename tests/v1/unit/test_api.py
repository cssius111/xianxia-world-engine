...
class APITester:
    ...
    def test_endpoint(
        self, method: str, endpoint: str, data: Dict[str, Any] = None, expected_status: int = 200
    ) -> Dict[str, Any]:
        """
        测试单个端点
            ...
            try:
                ...
                if "success" in response_data:
                    if response_data["success"]:
                        ...
                        print(
                            f"❌ 请求失败: {response_data.get('error', {}).get('message', '未知错误')}"
                        )
    ...
    def test_game_apis(self):
        ...
        # 执行命令
        self.test_endpoint("POST", "/game/command", data={"command": "帮助"})
    ...
    def test_save_apis(self):
        ...
        # 创建存档
        save_response = self.test_endpoint("POST", "/save/create", data={"name": "测试存档"})
        ...
        if save_response.get("success") and save_response.get("data"):
            ...
            if save_id:
                ...
                # 更新存档
                self.test_endpoint("PUT", f"/save/{save_id}", data={"name": "更新的存档名"})
...
def test_error_handling():
    ...
    # 测试无效的命令
    tester.test_endpoint("POST", "/game/command", data={"command": "abcdefg"}, expected_status=400)
