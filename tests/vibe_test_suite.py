...
class VibeTester:
        ...
        try:
            ...
            for input_text, expected_cmd, expected_params in test_cases:
                ...
                if cmd_match and params_match:
                    ...
                    print(f"❌ '{input_text}' → Expected: {expected_cmd.value}, Got: {result.command_type}")
                    all_pass = False
    ...
    def test_emotion_system(self) -> bool:
        ...
        try:
            from xwe.npc.emotion_system import EmotionState, EmotionType, EmotionSystem
    ...
    def generate_status_summary(self):
        ...
        print("\n🤖 AI FEATURES:")
        print(f"• NLP Command Parsing: {'✅ Working' if nlp_results.get('command_parsing') else '❌ Issues'}")
        print(f"• Duration Extraction: {'✅ Working' if nlp_results.get('duration_extraction') else '❌ Needs Fix'}")
