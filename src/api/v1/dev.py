...
def debug_info() -> Dict[str, Any]:
    ...
    try:
        ...
        return error_response(
            message=f"Failed to retrieve debug info: {str(e)}",
            code="DEBUG_ERROR",
            status_code=500
        )
    ...
    try:
        ...
        for event_name, listeners in event_dispatcher._listeners.items():
            listeners_info[event_name] = [
                {
                    "handler": str(listener),
                    "priority": getattr(listener, 'priority', 0)
                }
...
def debug_logs() -> Dict[str, Any]:
    ...
    try:
        ...
        if not log_service:
            return error_response(
                message="Log service not available",
                code="LOG_SERVICE_NOT_FOUND",
                status_code=404
            )
...
def _get_services_info(container: ServiceContainer) -> Dict[str, Any]:
    ...
    initialized_services = sum(
        1 for s in container._services.values() 
    ...
    return {
        ...
        "initialization_rate": f"{(initialized_services/total_services*100):.1f}%" if total_services > 0 else "0%"
    }
...
def _get_resource_usage() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": "Unable to retrieve resource usage"
        }
