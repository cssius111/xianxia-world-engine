...
class ISkillSystem(IGameSystem):
    ...
    @abstractmethod
    def use_skill(self, character: ICharacter, skill_id: str, 
                  targets: List[ICharacter]) -> 'ISkillResult':
        """使用技能"""
        pass
...
class ITimeSystem(IGameSystem):
    ...
    @abstractmethod
    def schedule_event(self, hours_from_now: int, event_name: str, 
...
class ICommandHandler(ABC):
    ...
    @abstractmethod
    def handle(self, command: str, args: Dict[str, Any], 
               context: 'IGameContext') -> 'ICommandResult':
        """处理命令"""
        pass
...
class IGameTime:
    ...
    def total_hours(self) -> int:
        """转换为总小时数"""
        return (self.year - 1) * 8760 + (self.month - 1) * 720 + \
               (self.day - 1) * 24 + self.hour
...
class ISkill(Protocol):
    ...
    def get_valid_targets(self, user: ICharacter, 
                         all_characters: List[ICharacter]) -> List[ICharacter]: ...
