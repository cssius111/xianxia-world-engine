...
class EventDispatcher:
    ...
    def on(self, event_name: str, handler: Callable, 
           priority: EventPriority = EventPriority.NORMAL,
           once: bool = False, weak: bool = False) -> Callable:
        """
        订阅事件
        ...
        if weak:
            ...
            weak_ref = weakref.ref(handler, 
                lambda ref: self._clean_weak_ref(event_name, ref))
    ...
    def off(self, event_name: str, handler: Callable) -> None:
        ...
        self.listeners[event_name] = [
            ...
            if h.handler != handler and 
               (not isinstance(h.handler, weakref.ref) or h.handler() != handler)
        ]
        
        # 从异步监听器中移除
        self.async_listeners[event_name] = [
            h for h in self.async_listeners[event_name]
            if h.handler != handler and
               (not isinstance(h.handler, weakref.ref) or h.handler() != handler)
        ]
        ...
        self._weak_refs[event_name] = {
            ref for ref in self._weak_refs[event_name]
            if ref() != handler
        }
    ...
    async def emit_async(self, event_name: str, 
            ...
            if isinstance(handler.handler, weakref.ref):
                ...
                if actual_handler:
                    tasks.append(asyncio.create_task(
                        asyncio.to_thread(actual_handler, event)
                    ))
            else:
                tasks.append(asyncio.create_task(
                    asyncio.to_thread(handler.handler, event)
                ))
    ...
    def emit_delayed(self, event_name: str, delay: float,
                    data: Optional[Dict[str, Any]] = None,
                    source: Optional[str] = None) -> asyncio.Task:
        """
        延迟发布事件
    ...
    def _get_handlers(self, event_name: str, 
                     sync_only: bool = True, 
                     async_only: bool = True) -> List[EventHandler]:
        """获取事件的所有处理器"""
        handlers = []
    ...
    def _clean_once_handlers(self, event_name: str) -> None:
        """清理一次性处理器"""
        self.listeners[event_name] = [
            h for h in self.listeners[event_name] if not h.once
        ]
        self.async_listeners[event_name] = [
