...
class MetricsCollector:
    ...
    def set_enabled(self, enabled: bool):
        ...
        with self._lock:
            ...
            logger.info(
                f"Prometheus metrics collection {'enabled' if enabled else 'disabled'}"
            )
    ...
    def record_context_compression(
        self, memory_blocks: int = 0, compression_ratio: float = 1.0
    ):
        """记录上下文压缩指标"""
        if not self._enabled:
            return
    ...
    def update_async_metrics(self, thread_pool_size: int = 0, queue_size: int = 0):
        """更新异步处理指标"""
    ...
    def record_command_execution(self, command: str, handler: str, duration: float):
        """记录命令执行时间"""
        if not self._enabled:
            return
    ...
    def record_api_call(self, api_name: str, endpoint: str, duration: float):
        ...
        with self._lock:
            try:
                api_call_latency.labels(api_name=api_name, endpoint=endpoint).observe(
                    duration
                )
            except Exception as e:
                logger.error(f"Failed to record API call: {e}")

    def update_game_metrics(self, instances: int = 0, players: int = 0):
        """更新游戏状态指标"""
        if not self._enabled:
            return
    ...
    def update_system_metrics(self, cpu_percent: float = 0.0, memory_mb: float = 0.0):
        """更新系统资源指标"""
        if not self._enabled or self._degraded:
            return
...
def init_prometheus_app_metrics(app, app_version="1.0.0", app_config=None):
    ...
    # 添加应用信息
    metrics.info(
        "xwe_app_info", "XianXia World Engine application info", version=app_version
    )
