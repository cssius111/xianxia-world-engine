...
class DialogueGenerator:
    ...
    def generate_greeting(self, context: DialogueContext) -> str:
        ...
        templates = self.greeting_templates.get(emotion, self.greeting_templates[EmotionType.NEUTRAL])
...
class EnhancedDialogueSystem:
    ...
    def __init__(self, dialogue_system: DialogueSystem, 
    ...
    def process_player_input(self, player_id: str, input_text: str) -> Tuple[Optional[DialogueNode], DialogueContext]:
        """
        处理玩家输入（自然语言）
    ...
    def advance_dialogue(self, player_id: str, choice_id: str) -> Tuple[Optional[DialogueNode], DialogueContext]:
        """
        推进对话（使用选项ID）
    ...
    def _generate_dynamic_response(self, context: DialogueContext, 
                                 input_text: str, intent: str) -> DialogueNode:
        """生成动态响应节点"""
        # 生成响应文本
    ...
    def _update_emotion_from_input(self, context: DialogueContext, input_text: str, intent: str) -> None:
        ...
        if triggered:
            self.emotion_system.trigger_emotion(context.npc_id, triggered, {
