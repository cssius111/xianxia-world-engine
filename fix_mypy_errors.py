    ...
    try:
        ...
        for i, line in enumerate(lines):
            if '|' in line and ('str' in line or 'Path' in line):
                ...
                # 处理 str | Path | None 模式
                new_line = re.sub(r'(\w+:\s*)str\s*\|\s*Path\s*\|\s*None', r'\1Optional[Union[str, Path]]', lines[i])
                if new_line != lines[i]:
                    lines[i] = new_line
        ...
        for i, line in enumerate(lines):
            ...
            if match and 'Optional' not in line:
                ...
                new_line = line.replace(f'{param_name}: {type_name}', f'{param_name}: Optional[{type_name}]')
                lines[i] = new_line
        ...
        if imports_needed:
            if typing_import_line >= 0:
                # 更新现有导入
                current_imports = re.findall(r'from typing import ([^\n]+)', lines[typing_import_line])[0]
