...
from xwe_v2.plugins.auction_system import AuctionItem, AuctionSystem, BidderType
...
class InteractiveAuction:
    ...
    def start_interactive_auction(self, player, items: List[AuctionItem]) -> Dict:
        """开始交互式拍卖"""
        results = {"items_won": [], "total_spent": 0, "events_triggered": []}

        print(self.visual.get_colored_text(f"\n{'='*60}\n拍卖会正式开始！\n{'='*60}\n", "YELLOW"))
            ...
            if item.current_bidder != player.name:
                ...
                print(
                    self.visual.get_colored_text(
                        f"\n您目前是最高出价者！({item.current_bid}灵石)", "GREEN"
                    )
                )
    ...
    def _display_item(self, item: AuctionItem, is_final: bool) -> None:
        ...
        print(self.visual.get_colored_text(f"起拍价：{item.base_price}下品灵石", "YELLOW"))
        ...
        for i in range(num_bids):
            ...
            if new_bid <= bidder.wealth * 0.1:  # 初始阶段保守
                ...
                archetype = next(
                    (
                        a
                        for a in self.auction_system.bidder_archetypes
                        if a["type"] == bidder.archetype
                    ),
                    None,
                )
                if archetype:
                    speech = random.choice(archetype["speech_patterns"]).replace(
                        "{price}", str(new_bid)
                    )
    ...
    def _get_player_decision(self, player, item: AuctionItem) -> str:
        """获取玩家决策"""
        min_increment = int(
            item.current_bid * self.auction_system.config["auction_rules"]["min_bid_increment"]
        )
    ...
    def _check_grudge_response(self, player, item: AuctionItem, player_bid: int) -> Optional[str]:
        ...
        for grudge in grudge_bidders:
            if random.random() < 0.6:  # 60%概率触发
                grudge_data = next(
                    (g for g in self.auction_system.grudge_npcs if g["name"] == grudge.name), None
                )
                if grudge_data:
    ...
    def _npc_respond_to_player_bid(self, item: AuctionItem, player_bid: int) -> bool:
        ...
        potential_bidders = [
            b
            for b in self.auction_system.bidders
    ...
    def _generate_npc_bid_dialogue(self, bidder, amount: int, is_response: bool = False) -> None:
        """生成NPC竞价对话"""
        archetype = next(
            (a for a in self.auction_system.bidder_archetypes if a["type"] == bidder.archetype),
            None,
        )
    ...
    def _npc_bidding_round(self, item: AuctionItem) -> bool:
        ...
        # 随机选择1-2个NPC出价
        active_bidders = [
            b for b in self.auction_system.bidders if b.wealth > item.current_bid * 1.1
        ]
    ...
    def _announce_result(self, item: AuctionItem, player_won: bool, final_price: int) -> None:
        ...
        if player_won:
            print(
                self.visual.get_colored_text(
                    f"\n恭喜您以{final_price}灵石成功拍得{item.name}！", "GREEN"
                )
            )
