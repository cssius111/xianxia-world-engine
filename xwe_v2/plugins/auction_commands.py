...
from xwe_v2.plugins.auction_system import auction_system
...
class AuctionCommandHandler:
    ...
    def _show_auction_help(self) -> str:
        ...
        output.append("\n基本规则：")
        output.append(
            f"- 最小加价幅度：当前价格的{int(auction_system.config['auction_rules']['min_bid_increment']*100)}%"
        )
        output.append(
            f"- 佣金率：成交价的{int(auction_system.config['auction_rules']['commission_rate']*100)}%"
        )
