...
try:
    from xwe_v2.plugins.visual_enhancement import VisualEnhancement
except Exception:  # pragma: no cover - fallback
    from xwe_v2.plugins.visual_enhancement import visual_effects
...
class AuctionSystem:
    ...
    def _get_default_config(self) -> Dict:
        ...
        return {
            "name": "天宝拍卖行",
            "auction_rules": {"min_bid_increment": 0.05, "commission_rate": 0.1},
        }
    ...
    def _generate_auction_items(self, auction_type: str) -> List[AuctionItem]:
        """生成拍卖物品列表"""
        items: List[Any] = []
        auction_config = self.config["auction_types"].get(
            auction_type, self.config["auction_types"]["regular"]
        )
        ...
        for tier, ratio in tier_dist.items():
            ...
            if tier_key in self.auction_items:
                tier_items = random.sample(
                    self.auction_items[tier_key], min(count, len(self.auction_items[tier_key]))
                )
    ...
    def _auction_item(
        self, player, item: AuctionItem, item_num: int, total_items: int
    ) -> List[str]:
        """拍卖单个物品"""
        output: List[str] = []
        ...
        # 展示物品信息
        output.append(self.visual.get_colored_text(f"\n【{item.name}】", "CYAN"))
        output.append(f"品阶：{item.tier}")
        output.append(f"描述：{item.description}")
        output.append(self.visual.get_colored_text(f"起拍价：{item.base_price}下品灵石", "YELLOW"))
        ...
        while consecutive_passes < 3 and round_num < 20:  # 最多20轮
            ...
            if item.current_bidder != player.name:
                min_increment = int(
                    item.current_bid * self.config["auction_rules"]["min_bid_increment"]
                )
        ...
        if item.current_bidder:
            output.append(
                self.visual.get_colored_text(
                    f"\n拍卖师：「{item.current_bid}灵石三次！成交！」", "GREEN"
                )
            )
            ...
            if item.current_bidder == player.name:
                ...
                output.append(
                    self.visual.get_colored_text(
                        f"恭喜您以{item.current_bid}灵石拍得{item.name}！", "GREEN"
                    )
                )
                player_won = True

                # 记录日志
                self.auction_log.append({"item": item.name, "price": item.current_bid, "won": True})
            else:
    ...
    def _generate_npc_bids(self, item: AuctionItem, player) -> List[Dict]:
        ...
        for bidder in self.bidders:
            ...
            if bidder.max_price == 0:
                variance = self.config["bidding_strategies"]["npc_behavior"][
                    "value_assessment_variance"
                ]
            ...
            if item.current_bid < bidder.max_price * 0.9:  # 还有10%余地
                ...
                if new_bid <= bidder.max_price and new_bid <= bidder.wealth:
                    # 根据性格决定是否出价
                    bid_chance = (
                        bidder.personality["aggression"] * 0.5
                        + bidder.personality["patience"] * 0.3
                    )

                    if random.random() < bid_chance:
                        # 生成竞价文本
                        archetype = next(
                            (a for a in self.bidder_archetypes if a["type"] == bidder.archetype),
                            None,
                        )
                        if archetype:
        ...
        for grudge in grudge_bidders:
            if random.random() < 0.7:  # 70%概率回应
                ...
                if grudge_data:
                    ...
                    spite_bid = int(
                        player_bid
                        * self.config["bidding_strategies"]["npc_behavior"][
                            "spite_bidding_multiplier"
                        ]
                    )
    ...
    def _trigger_auction_event(self) -> Optional[str]:
        ...
        for event_id, event_data in self.auction_events.items():
            if random.random() < event_data["probability"]:
                ...
                output.append(
                    self.visual.get_colored_text(
                        f"\n【特殊事件：{event_data['description']}】", "RED"
                    )
                )
    ...
    def process_player_bid(self, player, amount: int) -> str:
        ...
        min_increment = int(
            self.current_item.current_bid * self.config["auction_rules"]["min_bid_increment"]
        )
