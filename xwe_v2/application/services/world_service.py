...
class WorldService(ServiceBase[IWorldService], IWorldService):
    ...
    def get_map_info(self, location_id: str) -> Dict[str, Any]:
        ...
        if not location:
            return {"error": "未知位置", "description": "你不知道这个地方"}

        connections = self._connections.get(location_id, [])
        discovered_connections = [loc for loc in connections if loc in self._discovered_locations]
    ...
    def explore_location(self, location_id: str) -> Dict[str, Any]:
        ...
        location = self._locations.get(location_id)
        if not location:
            return {"success": False, "description": "未知位置"}
        ...
        if event_roll < 0.3 and location.get("enemies"):
            ...
            return {
                ...
                "encounter": {"type": "resource", "item": resource, "quantity": quantity},
            }

        elif event_roll < 0.6:
            # 发现新位置
            connections = self._connections.get(location_id, [])
            undiscovered = [loc for loc in connections if loc not in self._discovered_locations]
            ...
            if undiscovered:
                ...
                return {
                    ...
                    "encounter": {"type": "discovery", "location": new_location},
                }
        ...
        return {"success": True, "description": random.choice(descriptions), "encounter": None}
    ...
    def load_world_data(self, data: Dict[str, Any]) -> bool:
        ...
        self.logger.info(
            "World data loaded, %d locations discovered", len(self._discovered_locations)
        )
