...
from xwe_v2.events import EventBus, PlayerEvent
from xwe_v2.infrastructure.services import ServiceBase, ServiceContainer
from xwe_v2.infrastructure.services.interfaces.player_service import (
    IPlayerService,
    PlayerData,
)
...
class PlayerService(ServiceBase[IPlayerService], IPlayerService):
    ...
    def create_player(self, name: str, **kwargs) -> str:
        ...
        # 发布玩家创建事件
        self._publish_event(PlayerEvent("player_created", {"player_id": player_id, "name": name}))
    ...
    def update_player(self, player_id: str, updates: Dict[str, Any]) -> bool:
        ...
        # 发布更新事件
        self._publish_event(
            PlayerEvent("player_updated", {"player_id": player_id, "updates": updates})
        )
    ...
    def add_experience(self, amount: int) -> Dict[str, Any]:
        ...
        self._publish_event(
            PlayerEvent(
                "experience_gained",
                {"player_id": player.id, "amount": amount, "levels_gained": levels_gained},
            )
        )
    ...
    def level_up(self) -> bool:
        ...
        # 发布升级事件
        self._publish_event(
            PlayerEvent("player_level_up", {"player_id": player.id, "new_level": player.level})
        )
    ...
    def damage(self, amount: int) -> int:
        ...
        self._publish_event(
            PlayerEvent(
                "player_damaged",
                {"player_id": player.id, "damage": damage_dealt, "current_health": player.health},
            )
        )

        # 检查死亡
        if player.health <= 0:
            self._publish_event(PlayerEvent("player_died", {"player_id": player.id}))
    ...
    def use_mana(self, amount: int) -> bool:
        ...
        self._publish_event(
            PlayerEvent(
                "mana_used", {"player_id": player.id, "amount": amount, "current_mana": player.mana}
            )
        )
    ...
    def restore_mana(self, amount: int) -> int:
        ...
        if actual_restore > 0:
            self._publish_event(
                PlayerEvent(
                    "mana_restored",
                    {"player_id": player.id, "amount": actual_restore, "current_mana": player.mana},
                )
            )
    ...
    def add_skill(self, skill_id: str) -> bool:
        ...
        self._publish_event(
            PlayerEvent("skill_learned", {"player_id": player.id, "skill_id": skill_id})
        )
    ...
    def add_item(self, item_id: str, quantity: int = 1) -> bool:
        ...
        self._publish_event(
            PlayerEvent(
                "item_added", {"player_id": player.id, "item_id": item_id, "quantity": quantity}
            )
        )
    ...
    def remove_item(self, item_id: str, quantity: int = 1) -> bool:
        ...
        self._publish_event(
            PlayerEvent(
                "item_removed", {"player_id": player.id, "item_id": item_id, "quantity": quantity}
            )
        )
    ...
    def equip_item(self, item_id: str, slot: str) -> bool:
        ...
        self._publish_event(
            PlayerEvent(
                "item_equipped",
                {"player_id": player.id, "item_id": item_id, "slot": slot, "old_item": old_item},
            )
        )
