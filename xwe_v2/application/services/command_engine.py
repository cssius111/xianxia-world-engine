...
from xwe_v2.events import GameEvent, publish_event
    ...
    def handle(self, context: CommandContext) -> CommandResult:
        ...
        if len(context.args) < self.require_args:
            return CommandResult(
                success=False, output=f"命令格式错误。用法：{self.usage}", suggestions=[self.usage]
            )
        ...
        try:
            ...
            return CommandResult(success=False, output=f"命令执行出错：{str(e)}")
...
class ICommandEngine(ABC):
    ...
    def register_pattern_handler(
        self, pattern: str, handler: Callable[[CommandContext], CommandResult]
    ) -> None:
        """注册模式匹配处理器"""
        pass
...
class CommandEngine(ServiceBase[ICommandEngine], ICommandEngine):
    ...
    def process_command(self, command: str, **context_data) -> CommandResult:
        ...
        if not command:
            return CommandResult(
                success=False, output="请输入命令", suggestions=self.get_suggestions("")
            )
        ...
        # 创建上下文
        context = CommandContext(raw_command=command, command=cmd, args=args, **context_data)
        ...
        return result or CommandResult(
            success=False, output=f"未知命令：{cmd}", suggestions=self.get_suggestions(cmd)
        )
        ...
        if context.args:
            ...
            for handler in self.engine._handlers:
                if cmd_name.lower() in [
                    c.lower() for c in handler.commands
                ] or cmd_name.lower() in [a.lower() for a in handler.aliases]:
                    return CommandResult(success=True, output=handler.get_help())
