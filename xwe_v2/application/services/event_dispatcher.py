...
class IEventDispatcher(ABC):
    ...
    @abstractmethod
    def subscribe(
        self, event_type: str, handler: Callable[[DomainEvent], None], priority: int = 0
    ) -> None:
        """订阅事件"""
        pass
...
class EventDispatcher(ServiceBase[IEventDispatcher], IEventDispatcher):
    ...
    def _statistics_middleware(self, event: DomainEvent) -> DomainEvent:
        ...
        with self._lock:
            ...
            # 记录到队列
            self._event_queue.append({"timestamp": event.timestamp, "type": event.type})
    ...
    def subscribe(
        ...
        """订阅事件"""
        # 创建事件处理器包装
        from xwe_v2.events import FunctionEventHandler
    ...
    def get_event_history(
        ...
        """获取事件历史"""
        return self._event_store.get_events(event_type=event_type, limit=limit)
    ...
    def get_statistics(self) -> EventStatistics:
        ...
        with self._lock:
            ...
            if self._processing_times:
                self._statistics.average_processing_time = sum(self._processing_times) / len(
    ...
    def dispatch_player_event(
        self, event_type: str, data: Dict[str, Any], player_id: Optional[str] = None
    ) -> None:
        """分发玩家事件"""
        if player_id:
    ...
    def dispatch_combat_event(
        self, event_type: str, data: Dict[str, Any], combat_id: Optional[str] = None
    ) -> None:
        """分发战斗事件"""
        if combat_id:
    ...
    def dispatch_world_event(
        self, event_type: str, data: Dict[str, Any], location: Optional[str] = None
    ) -> None:
        """分发世界事件"""
        if location:
...
class SystemEventHandler(EventHandler):
    ...
    def _do_handle(self, event: DomainEvent) -> None:
        ...
        if event.type == "system_startup":
            ...
            self.logger.warning(
                f"Performance warning: {event.data.get('message', 'Unknown issue')}"
            )
