...
from xwe_v2.events import EventBus, GameEvent
...
class GameService(ServiceBase[IGameService], IGameService):
    ...
    def _do_initialize(self) -> None:
        """初始化服务"""
        # 延迟导入避免循环依赖
        from xwe_v2.infrastructure.services.interfaces.combat_service import (
            ICombatService,
        )
        from xwe_v2.infrastructure.services.interfaces.player_service import (
            IPlayerService,
        )
        from xwe_v2.infrastructure.services.interfaces.world_service import (
            IWorldService,
        )
    ...
    def initialize_game(self, player_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            self._publish_event(
                GameEvent(
                    "game_initialized",
                    {"location": self._current_location, "player_name": player_name},
                )
            )
...
"""
    ...
    def _handle_status(self) -> CommandResult:
        ...
        if not player:
            return CommandResult(success=False, output="未找到玩家信息")
...
"""
    ...
    def _handle_attack(self, command: str) -> CommandResult:
        ...
        if not self._in_combat:
            return CommandResult(
                success=False, output="当前不在战斗中", suggestions=["探索", "地图"]
            )
    ...
    def _handle_defend(self) -> CommandResult:
        """处理防御命令"""
        if not self._in_combat:
            return CommandResult(success=False, output="当前不在战斗中")
    ...
    def _handle_explore(self) -> CommandResult:
        ...
        if self._in_combat:
            return CommandResult(
                success=False, output="战斗中无法探索", suggestions=["攻击", "防御", "逃跑"]
            )
...
"""
    ...
    def _handle_cultivate(self) -> CommandResult:
        """处理修炼命令"""
        if self._in_combat:
            return CommandResult(success=False, output="战斗中无法修炼")

        # 执行修炼
        from xwe_v2.infrastructure.services.cultivation_service import (
            ICultivationService,
        )
        ...
        return CommandResult(
            success=True, output=result["message"], state_changed=True, events=[result]
        )
    ...
    def _start_combat(self, enemy_data: Dict[str, Any]) -> None:
        ...
        self._publish_event(
            GameEvent("combat_start", {"enemy": enemy_data, "location": self._current_location})
        )
    ...
    def save_game(self, save_name: str) -> bool:
        """保存游戏"""
        from xwe_v2.infrastructure.services.save_service import ISaveService
    ...
    def _add_log(self, log_type: str, message: str) -> None:
        """添加日志"""
        log_entry = {"timestamp": time.time(), "type": log_type, "message": message}
