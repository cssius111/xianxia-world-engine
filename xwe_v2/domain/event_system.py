...
from xwe_v2.core.formula_engine import evaluate_expression, formula_engine
from xwe_v2.infrastructure.persistence.data_manager import DM
from xwe_v2.plugins.deepseek_client import DeepSeekClient
...
class EventSystemV3:
    ...
    def _check_event_conditions(self, event: Dict[str, Any], context: Dict[str, Any]) -> bool:
        ...
        if trigger_type == "exploration":
            ...
            if time_condition and not self._check_time_condition(
                time_condition, context.get("game_time")
            ):
                return False
    ...
    def _has_completed_event(self, event_id: str) -> bool:
        """检查是否已完成某个事件"""
        return any(
            e.get("id") == event_id and e.get("completed", False) for e in self.event_history
        )
    ...
    def _create_event_instance(
        self, event_template: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """创建事件实例"""
        instance = {
        ...
        if not instance["choices"]:
            instance["choices"].append({"id": "continue", "text": "继续", "outcomes": []})
    ...
    def _process_description(self, description: str, context: Dict[str, Any]) -> str:
        ...
        if context.get("location"):
            description = description.replace(
                "{location_name}", context["location"].get("name", "未知地点")
            )
        ...
        if outcome_type == "reward":
            ...
            if "attributes" in rewards:
                for attr, value in rewards["attributes"].items():
                    setattr(player.attributes, attr, getattr(player.attributes, attr, 0) + value)
