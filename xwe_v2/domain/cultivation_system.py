...
from xwe_v2.core.formula_engine import calculate, formula_engine
from xwe_v2.infrastructure.persistence.data_manager import DM
...
class CultivationSystem:
    ...
    def attempt_breakthrough(self, player) -> Dict[str, Any]:
        ...
        返回突破结果
        """
        result = {"success": False, "message": "", "effects": {}, "penalties": {}}
        ...
        if roll < success_rate:
            ...
            # 记录突破次数
            player.cultivation["breakthrough_count"] = (
                player.cultivation.get("breakthrough_count", 0) + 1
            )
    ...
    def cultivate(self, player, duration_hours: int = 1) -> Dict[str, Any]:
        ...
        for hour in range(duration_hours):
            ...
            results["resource_consumed"]["mana"] = (
                results["resource_consumed"].get("mana", 0) + mana_cost
            )
            results["resource_consumed"]["stamina"] = (
                results["resource_consumed"].get("stamina", 0) + stamina_cost
            )

        # 应用经验
        player.exp += results["exp_gained"]
        player.cultivation["total_hours"] = (
            player.cultivation.get("total_hours", 0) + duration_hours
        )
    ...
    def _calculate_breakthrough_chance(
        self, player, current_realm: Dict[str, Any], next_realm: Dict[str, Any]
    ) -> float:
        """
        计算突破成功率
        ...
        for attr, value in benefits.items():
            if isinstance(value, str) and value.startswith("+"):
                ...
                if attr == "all_attributes":
                    ...
                    player.resources[f"max_{resource}"] = (
                        player.resources.get(f"max_{resource}", 0) + amount
                    )
                else:
