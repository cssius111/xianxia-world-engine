...
from xwe_v2.core.command_parser import CommandPattern, CommandType
...
class OptionsHandler(CommandHandler):
    ...
    def handle(self, context: CommandContext) -> CommandResult:
        ...
        config_data = {
            ...
            "自动保存": (
                f"每{config.auto_save_interval/60:.0f}分钟" if config.auto_save_enabled else "关闭"
            ),
...
class GameLauncherHandler(CommandHandler):
    ...
    def __init__(self, orchestrator):
        super().__init__("game_launcher", [], CommandPriority.SYSTEM)  # 不使用标准CommandType
        self.orchestrator = orchestrator
