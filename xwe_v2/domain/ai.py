...
from xwe_v2.domain.combat.models import CombatAction, CombatActionType, CombatState
from xwe_v2.domain.skills.models import SkillSystem, TargetType
...
class AIController:
    ...
    def _build_aggressive_tree(self) -> AIBehaviorNode:
        ...
        # 低血量时的紧急行为
        emergency = ConditionNode("emergency_check", lambda ctx: ctx["self_health_percent"] < 0.2)
        emergency.children.append(ActionNode("flee_or_heal", self._action_emergency_response))
        ...
        # 优先使用大招
        use_ultimate = ConditionNode("can_use_ultimate", lambda ctx: self._has_ultimate_ready(ctx))
        use_ultimate.children.append(ActionNode("use_ultimate", self._action_use_strongest_skill))
        attack_sequence.children.append(use_ultimate)

        # 常规攻击
        attack_sequence.children.append(ActionNode("normal_attack", self._action_aggressive_attack))
    ...
    def _build_defensive_tree(self) -> AIBehaviorNode:
        ...
        # 高威胁时防御
        high_threat = ConditionNode(
            "high_threat", lambda ctx: ctx["threat_level"] >= ThreatLevel.HIGH
        )
        high_threat.children.append(ActionNode("defend", self._action_defend))
        root.children.append(high_threat)

        # 治疗自己或队友
        need_heal = ConditionNode("need_healing", lambda ctx: self._need_healing(ctx))
        need_heal.children.append(ActionNode("heal", self._action_heal))
        root.children.append(need_heal)

        # 保守攻击
        root.children.append(ActionNode("conservative_attack", self._action_conservative_attack))
    ...
    def _build_balanced_tree(self) -> AIBehaviorNode:
        ...
        # 紧急情况
        emergency = ConditionNode("emergency_check", lambda ctx: ctx["self_health_percent"] < 0.3)
        ...
        # 根据情况选择行动
        good_opportunity = ConditionNode("good_opportunity", lambda ctx: self._has_advantage(ctx))
        ...
        # 默认攻击
        combat_selector.children.append(ActionNode("balanced_attack", self._action_balanced_attack))
    ...
    def _build_context(self, character: Character, combat_state: CombatState) -> Dict[str, Any]:
        ...
        injured_allies = [
            a for a in allies if a.attributes.current_health / a.attributes.max_health < 0.5
        ]
    ...
    def select_target(
        self, character: Character, enemies: List[Character], strategy: str = "highest_threat"
    ) -> Optional[Character]:
        """
        选择目标
    ...
    def _action_emergency_response(self, context: Dict[str, Any]) -> Optional[AIDecision]:
        ...
        # 尝试使用治疗技能
        heal_skills = [s for s in context["available_skills"] if s.skill_type.value == "heal"]
        if heal_skills:
            skill = heal_skills[0]
            return AIDecision(
                action=CombatAction(
        ...
        # 否则防御
        return AIDecision(
            action=CombatAction(action_type=CombatActionType.DEFEND, actor_id=character.id),
            priority=9,
            reasoning="紧急防御",
        )
    ...
    def _action_use_strongest_skill(self, context: Dict[str, Any]) -> Optional[AIDecision]:
        ...
        # 选择伤害最高的技能
        damage_skills = [
            s for s in context["available_skills"] if s.skill_type.value in ["attack", "control"]
        ]
    ...
    def _action_aggressive_attack(self, context: Dict[str, Any]) -> Optional[AIDecision]:
        ...
        # 优先使用技能
        attack_skills = [s for s in context["available_skills"] if s.skill_type.value == "attack"]
        ...
        return AIDecision(
            action=CombatAction(
                action_type=CombatActionType.ATTACK, actor_id=character.id, target_ids=[target.id]
            ),
            priority=5,
            reasoning="激进普通攻击",
        )
    ...
    def _action_heal(self, context: Dict[str, Any]) -> Optional[AIDecision]:
        ...
        if character.attributes.current_health / character.attributes.max_health < 0.5:
            ...
            target = min(
                injured_allies, key=lambda a: a.attributes.current_health / a.attributes.max_health
            )
            target_id = target.id
    ...
    def _action_heal_self(self, context: Dict[str, Any]) -> Optional[AIDecision]:
        ...
        heal_skills = [
            ...
            if s.skill_type.value == "heal" and s.target_type.value in ["self", "single_ally"]
        ]
    ...
    def _default_action(self, character: Character, combat_state: CombatState) -> CombatAction:
        ...
        # 没有敌人，等待
        return CombatAction(action_type=CombatActionType.WAIT, actor_id=character.id)
