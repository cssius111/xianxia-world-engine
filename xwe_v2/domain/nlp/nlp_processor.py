    ...
    def parse(self, user_input: str, context: Optional[Dict[str, Any]] = None) -> ParsedCommand:
        ...
        if not user_input.strip():
            return ParsedCommand(
                command_type=CommandType.UNKNOWN, raw_text=user_input, confidence=0
            )
        ...
        if self.config.enable_llm and self.config.api_key:
            try:
                ...
                if api_result and api_result.confidence > 0.3:
                    logger.info(
                        f"DeepSeek解析成功: {api_result.command_type} (置信度:{api_result.confidence})"
                    )
                    return api_result
...
Don't hold back. Give it your all. Be confident and concise."""
    ...
    def _parse_text_response(self, response_text: str, original_input: str) -> ParsedCommand:
        ...
        keyword_map = {
            ...
            ("breakthrough", "突破"): (
                getattr(CommandType, "BREAKTHROUGH", CommandType.UNKNOWN),
                0.8,
            ),
        }
        ...
        return ParsedCommand(
            command_type=CommandType.UNKNOWN, raw_text=original_input, confidence=0
        )
    ...
    def _extract_duration(self, text: str) -> Optional[str]:
        ...
        def _chinese_to_digit(zh: str) -> str:
            zh_digit = {
                "零": 0,
                "一": 1,
                "二": 2,
            ...
            if "十" in zh:
                parts = zh.split("十")
                left = zh_digit.get(parts[0], 1) if parts[0] else 1
                right = zh_digit.get(parts[1], 0) if len(parts) > 1 else 0
                return str(left * 10 + right)
            return str(zh_digit.get(zh, 1))

        match = re.search(r"([0-9]+|[一二三四五六七八九十两]+)\s*(?:个)?\s*([年月天日时])", text)
        if match:
            num = match.group(1)
            unit = match.group(2)
            num = _chinese_to_digit(num)
            return f"{num}{unit}"
    ...
    def _fuzzy_parse(self, text: str) -> ParsedCommand:
        ...
        # 状态查看
            ...
            return ParsedCommand(command_type=CommandType.STATUS, raw_text=text, confidence=0.95)
        ...
        # 背包物品
            ...
            return ParsedCommand(command_type=CommandType.INVENTORY, raw_text=text, confidence=0.9)

        # 地图
        elif any(w in text_lower for w in ["地图", "位置", "在哪", "哪里"]):
            return ParsedCommand(command_type=CommandType.MAP, raw_text=text, confidence=0.9)
        ...
        # 帮助
            ...
            return ParsedCommand(command_type=CommandType.HELP, raw_text=text, confidence=0.95)

        # 退出
        elif any(w in text_lower for w in ["退出", "离开", "再见", "拜拜", "quit", "exit"]):
            return ParsedCommand(command_type=CommandType.QUIT, raw_text=text, confidence=0.9)
    ...
    def _get_suggestions(self, text: str) -> list:
        ...
        # 默认建议
        if not suggestions:
            suggestions = ["查看状态", "查看地图", "修炼", "探索周围", "和附近的人聊天"]
