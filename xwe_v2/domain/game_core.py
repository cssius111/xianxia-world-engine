...
from xwe_v2.core.achievement_system import AchievementSystem
from xwe_v2.core.ai import AIController

# 导入优化系统
from xwe_v2.core.chinese_dragon_art import get_dragon_art, get_dragon_for_scene
from xwe_v2.core.command_parser import CommandParser, CommandType, ParsedCommand
from xwe_v2.core.command_router import CommandPriority, CommandRouter
from xwe_v2.core.data_loader import DataLoader
from xwe_v2.core.immersive_event_system import (
    EventType,
    ImmersiveEventSystem,
    SpecialEventHandler,
)
from xwe_v2.core.nlp import NLPConfig, NLPProcessor

# from xwe_v2.npc import NPCManager, DialogueSystem, TradingSystem  # 移到使用时导入，避免循环依赖
from xwe_v2.core.roll_system import CharacterRoller  # 导入Roll系统
from xwe_v2.core.status_manager import StatusDisplayManager
from xwe_v2.domain.character.attributes import AttributeSystem
from xwe_v2.domain.character.models import Character, CharacterType
from xwe_v2.domain.combat.models import (
    CombatAction,
    CombatActionType,
    CombatState,
    CombatSystem,
)
from xwe_v2.domain.inventory.models import Inventory
from xwe_v2.domain.items.models import item_system
from xwe_v2.domain.skills.models import SkillSystem
from xwe_v2.engine.expression import ExpressionParser
from xwe_v2.world import AreaType, EventSystem, LocationManager, TimeSystem, WorldMap
...
class GameCore:
    ...
    def __init__(
        self, data_path: Union[str, Path] | None = None, game_mode: str = "player"
    ) -> None:
        """
        初始化游戏核心
        ...
        # 初始化NPC系统
        # 局部导入避免循环依赖
        from xwe_v2.npc import DialogueSystem, NPCManager, TradingSystem
    ...
    def _display_roll_result(self, roll_result) -> None:
        ...
        self.output(
            f"攻击力：{attrs['attack']:<3} 防御力：{attrs['defense']:<3} 气血值：{attrs['health']}"
        )
        self.output(
            f"灵力值：{attrs['mana']:<3} 速度：{attrs['speed']:<3}   悟性：{attrs['comprehension']}"
        )
        self.output(
            f"气运：{attrs['luck']:<3}   根骨：{attrs['constitution']:<3} 魅力：{attrs['charm']}"
        )
        self.output("")
    ...
    def _handle_roll_choice(self, choice: str) -> None:
        ...
        if choice == "1":
            ...
            if self.current_roll_result:
                ...
                final_name = (
                    player_name if player_name != "无名侠客" else self.current_roll_result.name
                )

                self.game_state.player = self._create_player_from_roll(
                    final_name, self.current_roll_result
                )
                self._finalize_character_creation()
    ...
    def _finalize_character_creation(self) -> None:
        ...
        # 触发开局事件
        self.immersive_event_system.start_event(
            "start_journey",
            {"player": self.game_state.player, "location": self.game_state.current_location},
        )
    ...
    def _init_world(self) -> None:
        ...
        # 初始化默认地图
        from xwe_v2.domain.world.map import DEFAULT_MAP_DATA, Region
        ...
        for area_data in DEFAULT_MAP_DATA["areas"]:
            from xwe_v2.domain.world.map import Area
            ...
            for npc_data in npc_templates["npcs"][:3]:  # 只创建前3个
                ...
                if npc_id and npc_id in self.npc_manager.npc_profiles:
                    ...
                    if npc:
                        ...
                        logger.info(
                            f"创建NPC: {npc.name} 于 {profile.home_location if profile else '未知'}"
                        )
                else:
                    # 使用旧方法创建
                ...
                context["enemies"] = [
                    {
                        "id": e.id,
                        "name": e.name,
                        "health_percent": e.attributes.current_health / e.attributes.max_health,
                    }
                    for e in enemies
                    if e.is_alive
                ]
    ...
    def _process_combat_command(self, command: ParsedCommand) -> None:
        ...
        if command.command_type == CommandType.ATTACK:
            ...
            # 防御
            action = CombatAction(action_type=CombatActionType.DEFEND, actor_id=player.id)
            ...
            if self._try_flee():
                ...
                self.output("逃跑失败！")
                action = CombatAction(action_type=CombatActionType.WAIT, actor_id=player.id)

        # 执行玩家行动
        if action:
            result = self.combat_system.execute_action(self.game_state.current_combat, action)
    ...
    def _find_combat_target(
        self, combat_state: CombatState, target_name: str
    ) -> Optional[Character]:
        """在战斗中查找目标"""
    ...
    def _show_status(self) -> None:
        ...
        # 资源状态
        self.output(
            f"气血: {player.attributes.current_health:.0f}/{player.attributes.max_health:.0f}"
        )
        self.output(f"灵力: {player.attributes.current_mana:.0f}/{player.attributes.max_mana:.0f}")
        self.output(
            f"体力: {player.attributes.current_stamina:.0f}/{player.attributes.max_stamina:.0f}"
        )
        self.output("")
    ...
    def _show_map(self) -> None:
        ...
        if nearby_areas:
            ...
            for area_info in nearby_areas:
                ...
                self.output(
                    f"- {area_info['name']} ({area_info['type']}) " f"[危险: {danger}] [{status}]"
                )
    ...
    def _do_cultivate(self) -> None:
        ...
        exp_gained = SpecialEventHandler.handle_cultivation_event(
            self.immersive_event_system, {"attributes": player.attributes}, duration
        )
        ...
        self.achievement_system.check_achievement(
            "cultivation_100h", self.stats["cultivation_time"]
        )
    ...
    def _display_dialogue_node(self, node, npc_name: str) -> None:
        ...
        if node.type.value == "choice":
            ...
            choices = self.dialogue_system.get_active_dialogue(player.id).get_available_choices(
                context
            )
            if choices:
                self.output("\n请选择：")
    ...
    def _process_dialogue_choice(self, choice_id: str) -> None:
        ...
        if "npc_relationship" in context:
            self.npc_manager.npc_relationships[player.id][dialogue.npc_id] = context[
                "npc_relationship"
            ]
    ...
    def _create_monster(self, name: str, level: int) -> Character:
        """创建怪物"""
        monster = Character(name=name, character_type=CharacterType.MONSTER)
    ...
    def _show_combat_result(self, result, is_player: bool = True) -> None:
        ...
        for target_id, damage_info in result.damage_dealt.items():
            target = self.combat_system.get_combat(self.game_state.current_combat).participants.get(
                target_id
            )
            ...
            if target:
                if damage_info.is_evaded:
                    ...
                    if target.is_alive:
                        health_percent = (
                            target.attributes.current_health / target.attributes.max_health
                        )
    ...
    def _end_combat(self, combat_state: CombatState, fled: bool = False) -> None:
        ...
        if not fled:
            ...
            if winner == "player_team":
                ...
                self.achievement_system.check_achievement(
                    "warrior_10", self.stats["enemies_defeated"]
                )
                self.achievement_system.check_achievement(
                    "warrior_50", self.stats["enemies_defeated"]
                )
