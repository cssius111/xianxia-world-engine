...
from xwe_v2.domain.character.status import StatusEffectManager
...
class Character:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.spiritual_root:
            self.spiritual_root = {"金": 20, "木": 20, "水": 20, "火": 20, "土": 20}
    ...
    def can_act(self) -> bool:
        """是否可以行动"""
        return self.is_alive and self.state not in [CharacterState.STUNNED, CharacterState.FROZEN]
    ...
    def heal(self, amount: float) -> None:
        ...
        self.attributes.current_health = min(
            self.attributes.current_health + amount, self.attributes.max_health
        )
    ...
    def restore_mana(self, amount: float) -> None:
        ...
        self.attributes.current_mana = min(
            self.attributes.current_mana + amount, self.attributes.max_mana
        )
    ...
    def restore_stamina(self, amount: float) -> None:
        ...
        self.attributes.current_stamina = min(
            self.attributes.current_stamina + amount, self.attributes.max_stamina
        )
    ...
    def get_spiritual_root_description(self) -> str:
        ...
        # 按值排序
        sorted_roots = sorted(self.spiritual_root.items(), key=lambda x: x[1], reverse=True)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Character":
        ...
        character.attributes.current_health = attr_data.get(
            "current_health", character.attributes.current_health
        )
        character.attributes.current_mana = attr_data.get(
            "current_mana", character.attributes.current_mana
        )
        character.attributes.current_stamina = attr_data.get(
            "current_stamina", character.attributes.current_stamina
        )
