...
class SkillSystem:
    ...
    def calculate_skill_damage(
        self, skill: Skill, caster: Character, target: Character, effect: SkillEffect
    ) -> float:
        """
        计算技能伤害
        ...
        if effect.element and target.spiritual_root:
            damage *= self._calculate_element_bonus(effect.element, target.spiritual_root)

        return max(0, damage)

    def _calculate_element_bonus(
        self, attack_element: str, target_spiritual_root: Dict[str, float]
    ) -> float:
        """
        计算元素克制加成
        ...
        """
        # 简化的五行相克
        element_counters = {"金": "木", "木": "土", "土": "水", "水": "火", "火": "金"}
