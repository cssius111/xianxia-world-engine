...
from xwe_v2.core.trade_system import BlackMarket, MarketStall, Shopkeeper, TradeSystem
...
class TradeCommandHandler:
    ...
    def _init_shops(self) -> None:
        ...
        lingdan_shop.add_goods(
            "xiao_huan_dan", 30, self.trade_system.get_item_info("xiao_huan_dan")
        )
        ...
        lianqi_shop.add_goods(
            "qing_feng_dao_pao", 2, self.trade_system.get_item_info("qing_feng_dao_pao")
        )
        lianqi_shop.add_goods(
            "yi_jie_ju_ling_pan", 1, self.trade_system.get_item_info("yi_jie_ju_ling_pan")
        )
    ...
    def handle_buy_command(self, player: Character, item_index: int, quantity: int = 1) -> str:
        ...
        if player.get_total_lingshi() < total_cost:
            return (
                f"灵石不足！需要 {total_cost} 下品灵石。你只有 {player.get_lingshi_description()}。"
            )
    ...
    def handle_bargain_command(self, player: Character, item_index: int, offered_price: int) -> str:
        ...
        result, message = self.current_shop.bargain(item.item_id, offered_price, effective_charisma)
