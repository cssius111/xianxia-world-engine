...
class PluginManager:
    ...
    async def load_plugin(self, plugin_class: Type[Plugin], config: Optional[Dict] = None) -> None:
        """加载插件"""
    ...
    async def unload_plugin(self, plugin_name: str) -> None:
        ...
        for other_name, other_plugin in self.plugins.items():
            if plugin_name in other_plugin.dependencies:
                raise PluginError(f"Cannot unload {plugin_name}: " f"{other_name} depends on it")
    ...
    async def load_plugin_from_path(self, plugin_path: Path, config: Optional[Dict] = None) -> None:
        """从路径加载插件"""
    ...
    def _register_command(self, plugin_name: str, cmd_name: str, handler: Callable) -> None:
        """注册命令"""
    ...
    def _register_event_handler(self, plugin_name: str, event_type: str, handler: Callable) -> None:
        """注册事件处理器"""
    ...
    def _unregister_plugin_resources(self, plugin_name: str) -> None:
        ...
        if hasattr(self.engine, "commands"):
            ...
            commands_to_remove = [
                cmd
                for cmd in self.engine.commands.list_commands()
...
class ExamplePlugin(Plugin):
    ...
    def register_commands(self) -> Dict[str, Callable]:
        """注册命令"""
        return {"hello": self._hello_command}
