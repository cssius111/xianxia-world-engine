...
class PrometheusMetrics:
    ...
    def _register_core_metrics(self) -> None:
        ...
        # 在线玩家数
        self.register_gauge("active_players", "Number of active players")
        ...
        # API调用指标
        self.register_counter(
            "api_calls_total", "Total number of API calls", labels=["endpoint", "method"]
        )
    ...
    def register_counter(self, name: str, help: str, labels: Optional[List[str]] = None) -> None:
        ...
        with self._lock:
            self._metrics[name] = Metric(
                name=name, type=MetricType.COUNTER, help=help, labels=labels or []
            )
    ...
    def register_gauge(self, name: str, help: str, labels: Optional[List[str]] = None) -> None:
        ...
        with self._lock:
            self._metrics[name] = Metric(
                name=name, type=MetricType.GAUGE, help=help, labels=labels or []
            )
    ...
    def register_histogram(self, name: str, help: str, labels: Optional[List[str]] = None) -> None:
        ...
        with self._lock:
            self._metrics[name] = Metric(
                name=name, type=MetricType.HISTOGRAM, help=help, labels=labels or []
            )
    ...
    def _export_histogram(
        self, lines: List[str], name: str, observations: List[float], key: str
    ) -> None:
        """导出直方图数据"""
        if not observations:
            return
