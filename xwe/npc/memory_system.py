...
class MemorySystem:
    ...
    def __init__(self, max_memories_per_npc: int = 100) -> None:
        ...
        self.memory_clusters: Dict[str, Dict[str, MemoryCluster]] = (
            {}
        )  # npc_id -> cluster_id -> MemoryCluster
    ...
    def _find_related_memories(self, npc_id: str, new_memory: Memory) -> List[Memory]:
        ...
        for memory in self.npc_memories[npc_id].values():
            ...
            # 情感关联
            if (
                memory.emotion_association
                and new_memory.emotion_association
                and memory.emotion_association == new_memory.emotion_association
            ):
                score += 2
    ...
    def _update_memory_clusters(self, npc_id: str, memory: Memory) -> None:
        ...
        if best_cluster and best_score > 2:
            ...
            new_cluster = MemoryCluster(id=cluster_id, theme=theme, last_accessed=memory.game_time)
    ...
    def recall_memories(
        self, npc_id: str, context: Dict[str, Any], max_memories: int = 5
    ) -> List[Memory]:
        """
        回忆相关记忆
        ...
        for memory in memories.values():
            ...
            if (
                context.get("emotion")
                and memory.emotion_association
                and context.get("emotion") == memory.emotion_association
            ):
                score += 2
    ...
    def get_memory_summary(self, npc_id: str, subject: str) -> Dict[str, Any]:
        ...
        memories = [
            m for m in self.npc_memories[npc_id].values() if m.subject == subject and m.strength > 0
        ]
    ...
    def create_dialogue_memory(
        ...
        """创建对话记忆的便捷方法"""
        content = {"summary": dialogue_summary, "type": "dialogue"}
    ...
    def create_memory(
        self, npc_id: str, player_id: str, memory_type: str, game_time: int = 0, **content
    ) -> str:
        """兼容旧接口的创建记忆方法"""
        try:
