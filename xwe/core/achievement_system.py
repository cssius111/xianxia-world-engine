...
class Achievement:
    def __init__(
        self, id: str, name: str, description: str, icon: str, points: int, hidden: bool = False
    ):
        self.id = id
        self.name = name
...
class AchievementSystem:
    ...
    def _init_achievements(self) -> Dict[str, Achievement]:
        ...
        achievements = {
            # 新手成就
            "first_step": Achievement("first_step", "初入江湖", "完成角色创建", "🌟", 10),
            "first_battle": Achievement("first_battle", "初试锋芒", "赢得第一场战斗", "⚔️", 20),
            "first_cultivation": Achievement(
                "first_cultivation", "踏上仙途", "第一次修炼", "🧘", 15
            ),
            # 战斗成就
            "warrior_10": Achievement("warrior_10", "小有所成", "累计战胜10个敌人", "🗡️", 30),
            "warrior_50": Achievement("warrior_50", "百战之师", "累计战胜50个敌人", "⚔️", 50),
            "no_damage_win": Achievement(
                "no_damage_win", "毫发无伤", "在一场战斗中不受任何伤害并获胜", "🛡️", 40
            ),
            "win_streak_10": Achievement("win_streak_10", "连战连捷", "连续赢得10场战斗", "🔥", 45),
            # 修炼成就
            "breakthrough_qi": Achievement("breakthrough_qi", "筑基成功", "突破至筑基期", "💫", 50),
            "breakthrough_golden": Achievement(
                "breakthrough_golden", "金丹大成", "突破至金丹期", "🌟", 100
            ),
            "cultivation_100h": Achievement(
                "cultivation_100h", "勤修苦练", "累计修炼100小时", "⏰", 60
            ),
            # 探索成就
            "explorer_10": Achievement("explorer_10", "游历四方", "探索10个不同地点", "🗺️", 25),
            "secret_place": Achievement(
                "secret_place", "机缘巧合", "发现隐藏地点", "🎁", 35, hidden=True
            ),
            # 社交成就
            "friend_10": Achievement("friend_10", "广结善缘", "与10个NPC建立友好关系", "🤝", 30),
            "master_disciple": Achievement(
                "master_disciple", "名师高徒", "拜入师门或收徒", "👥", 40
            ),
            # 财富成就
            "rich_10000": Achievement("rich_10000", "小有积蓄", "拥有10000金币", "💰", 20),
            "rich_100000": Achievement("rich_100000", "富甲一方", "拥有100000金币", "💎", 50),
            # 特殊成就
            "death_escape": Achievement("death_escape", "大难不死", "从濒死状态恢复", "❤️‍🩹", 35),
            "legendary_item": Achievement(
                "legendary_item", "神兵在手", "获得传说级物品", "✨", 80, hidden=True
            ),
        }
...
"""
    ...
    def save_to_file(self, filepath: str) -> None:
        """保存成就数据"""
        data = {"total_points": self.total_points, "achievements": {}}

        for aid, achievement in self.achievements.items():
            data["achievements"][aid] = {
                "unlocked": achievement.unlocked,
                "unlock_time": (
                    achievement.unlock_time.isoformat() if achievement.unlock_time else None
                ),
