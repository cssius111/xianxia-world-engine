...
class CommandRouter:
    ...
    def _fuzzy_match(self, input_text: str) -> Tuple[str, Dict]:
        ...
        # 攻击命令模糊匹配
        attack_patterns = [r"^(攻击|打|揍|击败)\s*(.+)$", r"^(attack|hit|fight)\s+(.+)$"]
    ...
    def _handle_with_nlp(self, input_text: str) -> Tuple[str, Dict]:
        ...
        try:
            ...
            if nlp_result:
                return (nlp_result.get("command_type", "unknown"), nlp_result.get("parameters", {}))
        ...
        for priority, group_name in priority_groups.items():
            ...
            for cmd_name, cmd_def in self.commands.items():
                if cmd_def.priority == priority:
                    # 检查上下文
                    if (
                        cmd_def.context_required
                        and cmd_def.context_required != self.current_context
                    ):
                        continue
