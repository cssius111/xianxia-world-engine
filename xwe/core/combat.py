...
class CombatSystemV3:
    ...
    def calculate_damage(self, attacker, defender, action_type: str, 
        ...
        # 应用暴击
        if result["critical"]:
            crit_multi = self.combat_data["attack_resolution"]["hit_calculation"]["critical_hit"]["damage_multiplier"]
            damage *= crit_multi
        ...
        if skill_data and "effects" in skill_data:
            result["effects"] = self._process_skill_effects(skill_data["effects"], attacker, defender)
    ...
    def _select_target(self, character, combat_state: 'CombatState', 
                      priority: List[str], threats: Dict[str, float]) -> Optional[Any]:
        """选择目标"""
    ...
    def _select_skill(self, skills: List[Any], preference: str, 
                     character, target) -> Optional[Any]:
        """选择技能"""
        # 根据偏好过滤技能
    ...
    def _evaluate_skill_value(self, skill, character, target) -> float:
        ...
        # 控制技能
            ...
            # 根据目标威胁程度
            target_threat = target.attributes.get("attack_power", 0) * (target.health / target.max_health)
...
class Combat:
    ...
    def _calculate_initiative(self):
        ...
        for participant in self.participants.values():
            ...
            self.log(f"{participant.name} 先攻值: {total} (速度:{speed} + 骰子:{roll} + 加成:{initiative_bonus})")
    ...
    def execute_turn(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_type == ActionType.ATTACK:
            ...
            result = self._execute_skill(current_participant, action.get("skill"), action.get("target"))
    ...
    def _apply_skill_effect(self, caster, target, skill) -> Dict[str, Any]:
        ...
        if skill.effect_type == "damage":
            ...
            damage_result = self.system.calculate_damage(caster, target, damage_type, skill.to_dict())
...
class CombatState:
    ...
    def get_allies(self, character) -> List[Any]:
        ...
        for participant in self.participants.values():
            if getattr(participant, "team", -1) == team and participant.id != character.id and participant.health > 0:
