...
class CultivateHandler(CultivationHandler):
    ...
    def handle(self, context: CommandContext) -> CommandResult:
        ...
        context.state_manager.push_context(
            GameContext.CULTIVATING,
            {'start_time': context.state_manager.state.game_time}
        )
...
class BreakthroughHandler(CultivationHandler):
    ...
    def _calculate_breakthrough_rate(self, player) -> float:
        ...
        # 状态加成
        state_bonus = 0.1 if player.attributes.current_stamina == player.attributes.max_stamina else 0
    ...
    def _handle_breakthrough_success(self, context: CommandContext, player) -> None:
        ...
        new_realm = self.realm_names[player.attributes.realm + 1] if player.attributes.realm < 4 else "未知境界"
...
class CultivationCommandHandler(CommandHandler):
    ...
    def __init__(self):
        super().__init__(
            ...
            [CommandType.CULTIVATE, CommandType.LEARN_SKILL, 
             CommandType.BREAKTHROUGH, CommandType.USE_ITEM],
            CommandPriority.NORMAL
        )
