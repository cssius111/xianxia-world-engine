...
class DynamicDataManager:
    ...
    def cultivate_dynamic(self, duration_days: int = 1) -> Dict[str, Any]:
        ...
        for day in range(duration_days):
            ...
            if random.random() < 0.1:  # 10%概率
                ...
                if "attribute_changes" in event:
                    for attr, change in event["attribute_changes"].items():
                        ...
                        results["attribute_gains"][attr] = results["attribute_gains"].get(attr, 0) + change
            ...
            player["resources"]["spiritual_power"] = max(0, player["resources"]["spiritual_power"] - spiritual_cost)
            
            results["resource_changes"]["stamina"] = results["resource_changes"].get("stamina", 0) - stamina_cost
            results["resource_changes"]["spiritual_power"] = results["resource_changes"].get("spiritual_power", 0) - spiritual_cost
    ...
    def _can_breakthrough(self) -> bool:
        ...
        if current_realm in realm_requirements:
            ...
            return (self.player_data["level"] >= req["level"] and 
                    self.player_data["exp"] >= req["exp"])
