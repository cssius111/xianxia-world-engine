...
class ExpressionJITCompiler:
    ...
    def __init__(self) -> None:
        ...
        self.optimization_stats = {"compile_time": 0, "execution_speedup": []}
    ...
    def _expression_to_ast(self, expr: Any) -> ast.AST:
        ...
        if isinstance(expr, dict):
            if "constant" in expr:
                ...
                for part in parts:
                    node = ast.Subscript(value=node, slice=ast.Constant(value=part), ctx=ast.Load())
                return node

            elif "operation" in expr:
                op = expr["operation"]
                operands = [
                    self._expression_to_ast(operand) for operand in expr.get("operands", [])
                ]
                ...
                if op in ["+", "-", "*", "/", "//", "%", "**"]:
                    ...
                    for operand in operands[1:]:
                        result = ast.BinOp(left=result, op=op_map[op], right=operand)
                    return result

                # 函数调用
                elif op in ["min", "max", "abs", "sqrt"]:
                    return ast.Call(
                        func=ast.Name(id=op, ctx=ast.Load()), args=operands, keywords=[]
                    )
...
class ASTOptimizer(ast.NodeTransformer):
    ...
    def visit_Call(self, node) -> Any:
        ...
        # 常量函数调用折叠
        if isinstance(node.func, ast.Name) and all(
    ...
    def evaluate(self, expression: Any, context: Dict[str, Any]) -> Any:
        if isinstance(expression, dict):
            if "constant" in expression:
                ...
                operands = [
                    self.evaluate(operand, context) for operand in expression.get("operands", [])
                ]
