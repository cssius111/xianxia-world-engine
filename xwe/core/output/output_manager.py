...
class HTMLChannel(OutputChannel):
    ...
    def _generate_html(self) -> str:
        """生成HTML内容"""
        # 状态栏HTML
        status_items = [f"<li><strong>{k}</strong>: {v}</li>" for k, v in self.status_data.items()]
        status_html = "\n".join(status_items)
...
class OutputFormatter:
    ...
    @staticmethod
    def format_progress(
        current: float, total: float, width: int = 20, show_percentage: bool = True
    ) -> str:
        """格式化进度条"""
        if total == 0:
            percentage = 0
...
class OutputManager:
    ...
    def create_context(
        self, context_id: str, context_type: str, metadata: Optional[Dict[str, Any]] = None
    ) -> OutputContext:
        """创建输出上下文"""
        with self._lock:
            context = OutputContext(id=context_id, type=context_type, metadata=metadata or {})
    ...
    def achievement(self, content: str, **kwargs) -> None:
        """输出成就通知"""
        self.output(content, MessageType.ACHIEVEMENT, priority=MessagePriority.HIGH, **kwargs)

    def error(self, content: str, **kwargs) -> None:
        """输出错误信息"""
        self.output(content, MessageType.ERROR, priority=MessagePriority.HIGH, **kwargs)
    ...
    def debug(self, content: str, **kwargs) -> None:
        """输出调试信息"""
        self.output(content, MessageType.DEBUG, priority=MessagePriority.DEBUG, **kwargs)
    ...
    def output_status(self, status_dict: Dict[str, Any], title: str = "角色状态", **kwargs) -> None:
        """输出格式化的状态信息"""
    ...
    def output_progress(self, current: float, total: float, label: str = "", **kwargs) -> None:
        """输出进度条"""
    ...
    def get_history(
        self, count: int = 100, msg_type: Optional[MessageType] = None
    ) -> List[OutputMessage]:
        """获取历史消息"""
        with self._lock:
    ...
    def search_history(
        self, pattern: str, msg_type: Optional[MessageType] = None
    ) -> List[OutputMessage]:
        """搜索历史消息"""
