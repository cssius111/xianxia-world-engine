...
from xwe.core.output import (
    OutputManager, ConsoleChannel, FileChannel, HTMLChannel,
...
from xwe.core.world import World, Location
...
class GameOrchestrator:
    ...
    async def _init_output_manager(self) -> None:
        ...
        if self.config.game_mode != GameMode.DEV:
            ...
            if console:
                console.add_filter(
                    lambda msg: msg.type != MessageType.DEBUG
                )
    ...
    async def _init_command_processor(self) -> None:
        ...
        if self.config.enable_rate_limit:
            self.command_processor.add_middleware(
                RateLimitMiddleware(
                    max_commands=self.config.rate_limit_max,
...
"""
    ...
    def _on_level_up(self, data: Dict[str, Any]) -> None:
        ...
        self.output_manager.achievement(
            f"恭喜！你从 {old_level} 级升到了 {new_level} 级！"
        )
