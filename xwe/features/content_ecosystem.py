...
class ModLoader:
    ...
    def _create_mod_template(self) -> None:
        ...
        if not template_path.exists():
            ...
            with open(
                template_path / "npcs" / "mysterious_merchant.json", "w", encoding="utf-8"
            ) as f:
...
class ContentRegistry:
    ...
    def search_content(
        self, query: str, content_type: Optional[ContentType] = None
    ) -> List[ContentEntry]:
        """搜索内容"""
        results = []
...
"""
    ...
    def add_content_to_mod(
        self, mod_id: str, content_type: ContentType, content_data: Dict[str, Any]
    ) -> bool:
        """向MOD添加内容"""
        mod_path = self.mods_directory / mod_id
