...
"""
    ...
    async def generate_exploration_narrative(self,
                                           action: str,
        ...
        if cache_key in self.narrative_cache:
            return self._personalize_cached_narrative(
                self.narrative_cache[cache_key], 
                context
            )
    ...
    def _format_key_moments(self, key_moments: List[Dict]) -> str:
        ...
        for i, moment in enumerate(key_moments[:5]):  # 最多5个关键时刻
            if moment['type'] == 'critical':
                lines.append(f"{i+1}. {moment['attacker']}使用{moment['skill']}打出暴击，造成{moment['damage']}点伤害")
            elif moment['type'] == 'skill':
                lines.append(f"{i+1}. {moment['caster']}释放{moment['skill']}，{moment['effect']}")
            elif moment['type'] == 'near_death':
                lines.append(f"{i+1}. {moment['entity']}生命值降至{moment['health_percent']}%，濒临败北")
                
        return "\n".join(lines)
        
    def _build_exploration_prompt(self, action: str, discovery: Optional[Dict], 
                                context: dict) -> str:
        """构建探索提示"""
