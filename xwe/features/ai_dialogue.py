...
class AIDialogueManager:
    ...
    async def generate_npc_dialogue(self, 
        ...
        enhanced_context = self._enhance_context_with_history(
            context, npc_id, history
        )
        
        # 生成提示
        from xwe.core.nlp.advanced.prompt_engine import ResponseType, GameContext
    ...
    def _enhance_context_with_history(self, context: dict, npc_id: str, 
        ...
        if history:
            ...
            enhanced['dialogue_history'] = [
                {'speaker': h['speaker'], 'text': h['text'][:50] + '...'} 
    ...
    def _update_relationship(self, npc_id: str, dialogue_data: Dict):
        ...
        # 确保关系值在合理范围
        self.relationship_tracker[npc_id] = max(-100, min(100, 
                                              self.relationship_tracker[npc_id]))
