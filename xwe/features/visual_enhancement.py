...
class ProgressBar:
    ...
    def animate_to(
        self, target: int, duration: float = 1.0, prefix: str = "", suffix: str = ""
    ) -> None:
        """动画到目标值"""
        steps = 20
...
class VisualEffects:
    ...
    def display_combat_effect(self, attacker: str, target: str, damage: int) -> None:
        ...
        TextAnimation.flash(
            self.text_renderer.colorize(damage_text, "error"), times=2, interval=0.2
        )
    ...
    def display_item_obtained(self, item_name: str, item_type: str = "common") -> None:
        """获得物品效果"""
        color_map = {"common": "normal", "rare": "info", "epic": "warning", "legendary": "accent"}

        color_type = color_map.get(item_type, "normal")

        print()
        TextAnimation.flash(self.text_renderer.colorize("获得物品！", "success"), times=2)
    ...
    def display_status_bar(
        self, hp: int, max_hp: int, mp: int, max_mp: int, exp: int, max_exp: int
    ) -> None:
        """显示状态条"""
        # 气血值
    ...
    def display_achievement(self, achievement_name: str, description: str) -> None:
        ...
        print()
        TextAnimation.flash(
            self.text_renderer.colorize("🏆 成就解锁！", "success"), times=3, interval=0.3
        )

        achievement_box = self.text_renderer.box(
            f"{achievement_name}\n\n{description}", style="double"
        )
