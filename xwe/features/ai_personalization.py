...
class PlayerProfile:
    ...
    def get_recent_behaviors(self, time_window: float = 3600) -> List[PlayerBehavior]:
        ...
        return [b for b in self.behavior_history if current_time - b.timestamp <= time_window]
...
class PlayerStyleAnalyzer:
    ...
    def analyze_player_style(
    ...
    ) -> Tuple[PlayerStyle, Optional[PlayerStyle]]:
        """分析玩家风格"""
        current_time = time.time()
        ...
        secondary_style = (
            sorted_styles[1][0] if len(sorted_styles) > 1 and sorted_styles[1][1] > 0.15 else None
        )
        ...
        logger.info(
            f"玩家风格分析: 主要={primary_style.value}, 次要={secondary_style.value if secondary_style else 'None'}"
        )
    ...
    def analyze_content_preferences(self, profile: PlayerProfile) -> Dict[ContentPreference, float]:
        ...
        if total_count > 0:
            preferences = {pref: count / total_count for pref, count in preference_counts.items()}
        else:
            # 默认偏好
...
class AdaptiveGuideSystem:
    ...
    def get_content_recommendations(self, profile: PlayerProfile) -> List[Dict[str, Any]]:
        ...
        if profile.primary_style in self.guide_templates:
            ...
            for rec in random.sample(style_recs, min(2, len(style_recs))):
                recommendations.append({"type": "guide", "content": rec, "priority": "high"})

        # 基于内容偏好的推荐
        top_preferences = sorted(
            profile.content_preferences.items(), key=lambda x: x[1], reverse=True
        )[:3]
    ...
    def suggest_next_action(
        self, profile: PlayerProfile, game_context: Dict[str, Any]
    ) -> Optional[str]:
        """建议下一步行动"""
        suggestions = []
...
class DynamicNPCBehavior:
    ...
    def generate_npc_dialogue(
        ...
        """生成个性化NPC对话"""
        personality = self.npc_personalities.get(
            npc_personality, self.npc_personalities["friendly"]
        )

        # 获取NPC状态
        npc_state = self.npc_states.get(
            npc_id,
            {"mood": "neutral", "last_interaction": 0, "interaction_count": 0, "relationship": 0},
        )
    ...
    def npc_should_approach_player(
        self, npc_id: str, npc_personality: str, player_profile: PlayerProfile
    ) -> bool:
        """判断NPC是否应该主动接近玩家"""
        personality = self.npc_personalities.get(
            npc_personality, self.npc_personalities["friendly"]
        ...
        if player_nearby and player_profile:
            ...
            if random.random() < personality["help_probability"]:
                if player_profile.primary_style == PlayerStyle.EXPLORER:
                    actions.append(
                        {"type": "offer_information", "content": "我知道一些你可能感兴趣的地方..."}
                    )
                elif player_profile.primary_style == PlayerStyle.ACHIEVER:
                    actions.append(
                        {"type": "offer_quest", "content": "我这里有个任务，你或许能帮上忙..."}
                    )
        else:
            # 日常行为
            daily_actions = ["patrol", "idle", "work", "chat_with_npc", "move_to_location"]
            actions.append(
                {"type": random.choice(daily_actions), "duration": random.randint(30, 300)}
            )
...
class PersonalizationEngine:
    ...
    def get_personalized_content(self, player_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "preferred_content": [
                pref.value for pref, score in profile.content_preferences.items() if score > 0.2
            ],
        }
    ...
    def generate_adaptive_story(self, player_id: str, story_context: Dict[str, Any]) -> str:
        ...
        # 根据玩家偏好调整剧情元素
        if (
            ContentPreference.COMBAT in profile.content_preferences
            and profile.content_preferences[ContentPreference.COMBAT] > 0.3
        ):
            base_story += "\n剧情中包含激烈的战斗场面。"

        if (
            ContentPreference.DIALOGUE in profile.content_preferences
            and profile.content_preferences[ContentPreference.DIALOGUE] > 0.3
        ):
            base_story += "\n剧情中有深入的角色对话。"
    ...
    def export_profile_data(self, player_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "content_preferences": {
                pref.value: score for pref, score in profile.content_preferences.items()
            },
...
def enhance_with_ai_features(game_core) -> None:
    ...
    def ai_enhanced_process_command(input_text: str) -> None:
        ...
        personalization_engine.record_player_action(
            player_id=player_id, action_type=action_type, context={"input": input_text}
        )
    ...
    game_core.get_ai_recommendation = lambda: personalization_engine.get_personalized_content(
        "player_1"
    )
