...
class LocationManager:
    ...
    def move_entity(self, entity_id: str, target_area_id: str, 
                   player_level: int = 0) -> Tuple[bool, str]:
        """
        移动实体到目标区域
        ...
        # 检查是否可以移动
        can_move, reason = self.world_map.can_move_to(
            current_area_id, target_area_id, player_level
        )
    ...
    def get_area_description(self, entity_id: str) -> str:
        ...
        # 添加其他实体
        other_entities = [e for e in self.get_entities_in_area(current_area_id) 
                         if e != entity_id]
        if other_entities:
