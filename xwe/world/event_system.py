...
def _load_item_ids() -> List[str]:
    ...
    try:
        data_path = (
            Path(__file__).resolve().parents[1] / "data" / "restructured" / "item_template.json"
        )
...
class EventSystem:
    ...
    def _init_default_events(self) -> None:
        ...
        beast_attack = WorldEvent(
            ...
            conditions=[
                EventCondition(EventTrigger.LOCATION, {"location_type": "wilderness"}),
                EventCondition(EventTrigger.RANDOM, {"chance": 0.15}),
            ],
            intro_text="你在荒野中行走时，突然从草丛中跳出一只低阶妖兽，"
            "它双眼发红，显然已经饿了很久。",
            choices=[
                EventChoice(
                    id="fight", text="与妖兽战斗", consequences={"start_combat": "low_beast"}
                ),
                EventChoice(
                    id="flee",
                    text="立即逃跑",
        ...
        find_treasure = WorldEvent(
            ...
            conditions=[
                EventCondition(EventTrigger.ACTION, {"action": "explore"}),
                EventCondition(EventTrigger.RANDOM, {"chance": 0.05}),
            ],
            intro_text="在探索过程中，你注意到一块奇怪的石头。"
            "移开石头后，你发现了一个古老的储物袋！",
            choices=[
                EventChoice(id="open", text="打开储物袋", consequences={"add_random_item": True}),
                EventChoice(
                    id="check_trap",
                    text="先检查是否有陷阱",
        ...
        mysterious_merchant = WorldEvent(
            ...
            conditions=[
                EventCondition(EventTrigger.LOCATION, {"location_type": "market"}),
                EventCondition(EventTrigger.RANDOM, {"chance": 0.1}),
            ],
            intro_text="一位身穿黑袍的神秘商人悄悄靠近你，"
            "低声说道：'年轻人，我这里有些特别的东西...'",
            choices=[
                EventChoice(id="browse", text="查看商品", consequences={"open_special_shop": True}),
                EventChoice(
                    id="ask_info",
                    text="询问消息",
                ...
                EventChoice(id="leave", text="礼貌拒绝", consequences={}),
            ],
            repeatable=True,
            max_occurrences=999,
        )
    ...
    def load_events_from_file(self, filepath: str) -> None:
        ...
        try:
            ...
            for event_data in data.get("events", []):
                try:
                    conditions = [
                        EventCondition(EventTrigger(cond.get("type")), cond.get("params", {}))
    ...
    def handle_choice(
        self, event_id: str, choice_id: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        处理事件选择
        ...
        # 处理选择后果
        result = {"success": True, "consequences": choice.consequences, "message": ""}
