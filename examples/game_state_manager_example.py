...
from xwe.core.state import GameContext, GameState, GameStateManager
...
def example_context_management():
    ...
    # 进入对话
    state_manager.push_context(
        GameContext.DIALOGUE, {"npc_id": "elder_wang", "dialogue_node": "greeting"}
    )
    print(f"当前上下文: {state_manager.get_current_context().name}")
    print(f"对话数据: {state_manager.get_context_data()}")

    # 在对话中进入交易
    state_manager.push_context(GameContext.TRADING, {"shop_id": "elder_wang_shop", "discount": 0.9})
...
def example_combat_flow():
    ...
    # 结束战斗
    combat_result = {"winner": "player", "exp_gained": 100, "gold_gained": 50, "duration": 5}
...
def example_snapshots():
    ...
    print(
        f"初始状态 - 位置: {state_manager.get_location()}, 金币: {state_manager.get_flag('gold')}"
    )
    ...
    print(
        f"被抢劫后 - 位置: {state_manager.get_location()}, 金币: {state_manager.get_flag('gold')}"
    )

    # 恢复到上一个快照
    state_manager.restore_snapshot(-1)
    print(
        f"撤销一步 - 位置: {state_manager.get_location()}, 金币: {state_manager.get_flag('gold')}"
    )

    # 恢复到第一个快照
    state_manager.restore_snapshot(0)
    print(
        f"撤销到开始 - 位置: {state_manager.get_location()}, 金币: {state_manager.get_flag('gold')}"
    )
