...
class SimpleGame:
    ...
    def __init__(self):
        self.player = Character(
            name="张三", character_type=CharacterType.PLAYER, charisma=50, bargain_skill=1
        )
    ...
    def display_status(self):
        ...
        print(
            f"背包：{self.player.inventory.get_used_slots()}/{self.player.inventory.max_slots} 格"
        )
        print("=" * 40)
            ...
            if cmd in ["购买", "buy"]:
                if len(parts) >= 2:
                    try:
                        ...
                        result = self.trade_handler.handle_buy_command(
                            self.player, item_index, quantity
                        )
                        print(result)
                    except ValueError:
                        print("请输入有效的数字。")
                ...
                if len(parts) >= 2:
                    item_name = parts[1]
                    quantity = int(parts[2]) if len(parts) >= 3 else 1
                    result = self.trade_handler.handle_sell_command(
                        self.player, item_name, quantity
                    )
                    print(result)
                else:
                    print("用法：出售 [物品名] [数量]")
                ...
                if len(parts) >= 3:
                    try:
                        item_index = int(parts[1])
                        price = int(parts[2])
                        result = self.trade_handler.handle_bargain_command(
                            self.player, item_index, price
                        )
                        print(result)
                    except ValueError:
                        print("请输入有效的数字。")
