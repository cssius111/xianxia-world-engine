...
from xwe.core.orchestrator import GameConfig, GameMode, GameOrchestrator
...
class GameCoreRefactored:
    ...
    def save_game(self, filename: str) -> bool:
        ...
        try:
            return loop.run_until_complete(self.orchestrator.save_game(filename))
        finally:
            loop.close()
    ...
    def load_game(self, filename: str) -> bool:
        ...
        try:
            return loop.run_until_complete(self.orchestrator.load_game(filename))
        finally:
            loop.close()
...
def demonstrate_refactored_core():
    ...
    # 创建角色
    print("\n创建角色...")
    player = game.create_character("测试仙人", level=5, max_health=200, max_mana=100)
