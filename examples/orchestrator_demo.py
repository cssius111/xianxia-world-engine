...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("game_demo.log"), logging.StreamHandler()],
    )
...
def demo_game_scenarios():
    ...
    async def setup_demo_scenario(orchestrator):
        ...
        # 创建一个高级角色用于演示
        demo_player = Character(name="演示道长", character_type=CharacterType.PLAYER)
        ...
        # 添加演示命令
        from xwe.core.command import CommandHandler, CommandPriority, CommandResult
        ...
        class DemoCommandHandler(CommandHandler):
            ...
            def _demo_dialogue(self, context):
                ...
                context.output_manager.dialogue(
                    "神秘老者", "年轻人，我看你骨骼清奇，可愿意接受我的考验？"
                )
                context.output_manager.menu(
                    ["愿意接受考验", "询问考验内容", "婉言谢绝"], "选择回应"
                )

                # 进入对话状态
                context.state_manager.push_context(
                    context.state_manager.GameContext.DIALOGUE, {"npc": "神秘老者"}
                )
