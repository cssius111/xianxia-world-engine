...
from xwe.core.output import OutputManager, ConsoleChannel, FileChannel, HTMLChannel, MessageType
...
class GameCoreIntegrationExample:
    ...
    def _setup_output_channels(self):
        ...
        if self.state_manager.state.game_mode == "dev":
            ...
            self.output_manager.add_channel(HTMLChannel(
                html_path,
                title="游戏调试输出",
                auto_refresh=1
            ))
    ...
    def _process_combat_action_new(self, action: str, damage: int, target: str):
        ...
        # 批量输出战斗信息
        self.output_manager.combat_sequence(
            combat_messages,
            context_id=self.current_combat_context
        )
    
    def _do_talk_new(self, npc_name: str):
        """使用新架构的对话系统"""
        # 进入对话上下文
        self.state_manager.push_context(
            GameContext.DIALOGUE,
            {'npc_name': npc_name}
        )
    ...
    def _on_quest_completed(self, data):
        ...
        self.output_manager.achievement(
            f"任务完成：{quest.get('name', quest_id)}",
            context_id=ctx_id
        )
